...
一个轻量级的本地密码管理工具，基于 easygui 图形界面。
...
| 功能 | 说明 |
|------|------|
| 🔐 生成密码 | 可配置长度 (4-64) 和字符集（大小写、数字、符号） |
| ✏️ 输入密码 | 手动输入密码并保存 |
| 🔄 修改密码 | 5 种模式：手动/随机/纯数字/纯字母/数字 + 字母 |
...
| `change_password()` | 修改密码，支持 5 种生成模式 |
...
| 组件 | 用途 |
|------|------|
| `integerbox()` | 密码长度数字输入（4-64） |
| `multchoicebox()` | 字符类型多选（大写/小写/数字/符号） |
| `msgbox()` | 显示密码结果和提示信息 |
| `multenterbox()` | 输入服务名称和用户名 |
| `choicebox()` | 密码列表选择查看 |
| `ccbox()` | 确认对话框（保存/复制/删除） |
...
- 所有密码数据存储在本地 SQLite 数据库
- 使用 `secrets` 模块生成加密安全的随机密码
- 参数化查询防止 SQL 注入
- 密码明文存储，建议配合系统加密或加密容器使用
